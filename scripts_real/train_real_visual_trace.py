...
from iclr.data.dataset_real_visual_trace import SequenceDataset_Real_Visual_Trace

import iclr.util.misc as misc
from iclr.util.misc import NativeScalerWithGradNormCount as NativeScaler
from iclr.util.args import ExperimentConfig
from iclr.util.engine_real import train_one_epoch_real_visual_trace
from iclr.util.model_constructor_real import model_constructor_real_visual_trace

def main(args : ExperimentConfig):
    misc.init_distributed_mode(args)    # initialize the distributed mode
    ...
    # make sure the number of cameras is correct, we have a side camera and a wrist camera
    rgb_observations = data_cfg["image_keys"]   # 2 cameras
    ...
    # no_aug_vision_transform is Compose(
    ...
    # base_lr is "reference learning rate" that gets automatically scaled based on the batch size
    ...
    dataset_train = SequenceDataset_Real_Visual_Trace(
    ...
    dataset_val = SequenceDataset_Real_Visual_Trace(
        dataset_config=args.dataset_cfg,
        shared_config=args.shared_cfg,
        vision_transform=vision_transform,
        no_aug_vision_transform=no_aug_vision_transform,
    ...
    for epoch in range(args.shared_cfg.start_epoch, args.trainer_cfg.epochs):
        ...
        train_stats = train_one_epoch_real_visual_trace(  # main function of training
        ...
        if data_loader_val is not None:
            with torch.no_grad():
                val_stats = train_one_epoch_real_visual_trace(
