...
def train_one_epoch_real_visual_trace(model: ICLR_Real, 
                    data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler,
                    log_writer=None, validate=False,    # validate is used to evaluate the model on the val set
    ...
    for data_iter_step, dataset_item in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
            loss, loss_dict = model(dataset_item)   # major function of training
