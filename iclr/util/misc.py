...
class SmoothedValue(object):
    ...
    window or the global series average. This is used for logging the metrics.
    """
    ...
    def synchronize_between_processes(self):
        ...
        dist.barrier()
        dist.all_reduce(t)  # reduce (sum) the tensor across all processes
        t = t.tolist()
        self.count = int(t[0])
        self.total = t[1]
...
class MetricLogger(object):
    ...
    def update(self, **kwargs):
        for k, v in kwargs.items():
            ...
            self.meters[k].update(v)    # this calls the update method of the SmoothedValue class
...
def setup_for_distributed(is_master):
    ...
    It replaces the built-in print function to control which processes can print output during distributed training.
    """
...
def init_distributed_mode(args):
    if args.dist_on_itp:    # always False
        args.rank = int(os.environ['OMPI_COMM_WORLD_RANK'])
        args.world_size = int(os.environ['OMPI_COMM_WORLD_SIZE'])
        args.gpu = int(os.environ['OMPI_COMM_WORLD_LOCAL_RANK'])
        args.dist_url = "tcp://%s:%s" % (os.environ['MASTER_ADDR'], os.environ['MASTER_PORT'])
        os.environ['LOCAL_RANK'] = str(args.gpu)    # never used
    ...
    elif 'SLURM_PROCID' in os.environ:  # never happens
        args.rank = int(os.environ['SLURM_PROCID'])
        args.gpu = args.rank % torch.cuda.device_count()
    else:   # not using distributed training
