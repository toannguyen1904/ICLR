...
from iclr.models.shared.llama import Transformer, ModelArgs
from iclr.models.shared.pred_head import MLPHead, GMMHead, DiffusionHead
...
class ICLR_Real(nn.Module):
    ...
    # adding diffusion hyperparameters  # never used as we are not using diffusion
    ...
    def __init__(
        ...
        vision_encoder : Union[VisionEncoder, VisionEncoderCNN], # We are using VisionEncoder
        phase : str = "pretrain", # phase of training
        num_cameras : int = 2,  # number of cameras, we have a side camera and a wrist camera
        ...
        random_mask_visual_trace : bool = True,
    ):
        super().__init__()
        ...
        self.latent_dim = model_args.dim    # 768
        print(f"language model args: {model_args}")

        # set up diffusion parameters if they are defined, they are not defined
        ...
        # vision encoder 
        self.vision_encoder = vision_encoder    # type iclr.models.backbones.encoders.VisionEncoder
        self.vision_out_dim = self.vision_encoder.out_dim() # 768
        self.vision_finetune = vision_encoder.finetune # False
        
        # randomly initialize camera positional embeddings
        self.num_cameras = num_cameras
        self.camera_pos_emb = camera_pos_emb    # False
        if camera_pos_emb:
            self.iclr_camera_pos_emb = nn.Parameter(torch.randn(num_cameras, 1, self.vision_out_dim))
        ...
        if not self.remove_proprio: # since we are not removing proprio, we need to encode it
            self.iclr_proprio_encoder = Mlp(in_features=self.proprio_dim, out_features=self.vision_out_dim) # self.vision_out_dim is 768

        # attention pooling 
        assert self.attn_latent_len == 1, "currently causal attention only supports attn_latent_len=1"
        attn_pool_cls = MultiKVAttentionPool if multikv_attn_pool else AttentionPool    # multikv_attn_pool is False, so use AttentionPool

        self.separate_camera_adapter = separate_camera_adapter and num_cameras > 1  # True
        if self.separate_camera_adapter:  # True
            self.iclr_attn_pooling = nn.ModuleList(
                [attn_pool_cls(
            ...
            self.padding = self.latent_dim - self.latent_dim // num_cameras * num_cameras   # padding is 0
        else:
            self.iclr_attn_pooling = attn_pool_cls(
                self.vision_out_dim, out_features=self.latent_dim, mlp_ratio=adapter_mlp_ratio,
                latent_len=self.attn_latent_len, num_heads=adapter_num_heads,
            )
        self.iclr_vision_norm = nn.LayerNorm(normalized_shape=self.vision_out_dim, eps=1e-6) # consistent with timm ViT Formulation
        
        # flag for prediction action only: if this is true, no proprio prediction is needed
        self.pred_action_only = pred_action_only    # True
        self.no_prompt_loss = no_prompt_loss    # False
        
        self.step_weight = step_weight  # 1.0

        # decoding to proprio space
        self.num_pred_steps = num_pred_steps    # 16
        if not self.pred_action_only:
            self.action_dim = action_dim    # 11
        else: 
            # we do not predict eos anymore 
            self.action_dim = action_dim - 1    # 10, we use this
        ...
        if self.modality_pos_emb:
            ...
            self.iclr_pos_emb_state = nn.Parameter(torch.randn(self.latent_dim))
            self.iclr_pos_emb_act = nn.Parameter(torch.randn(self.latent_dim))
            self.iclr_pos_emb_visual_trace = nn.Parameter(torch.randn(self.latent_dim))

            # initialize them 
            nn.init.trunc_normal_(self.iclr_pos_emb_state, std=0.2)
            nn.init.trunc_normal_(self.iclr_pos_emb_act, std=0.2)
            nn.init.trunc_normal_(self.iclr_pos_emb_visual_trace, std=0.2)
        # constructing decoder for proprio and action 
        self.decoder_pred_head = decoder_pred_head  # "mlp"
        ...
        # we enforce loading to llama checkpoint, currently unused
        ...
        self.kl_div_loss = kl_div_loss   # False
        ...
        # 7. weighs 
        if self.pred_action_only:   # True
            # we assign no weight to proprio and eos prediction
            self.loss_w_proprio = 0
        
        self.scale_loss = scale_loss    # 1.0
        total_weight = sum([loss_w_visual_trace, loss_w_action, self.loss_w_proprio, self.loss_w_kl])
        self.loss_w_action = loss_w_action / total_weight
        self.loss_w_visual_trace = loss_w_visual_trace / total_weight
        ...
        self.scale_action = scale_action    # None
    ...
    def _construct_decoder_heads(self):
        ...
        if not self.pred_action_only:   # self.pred_action_only is True
            self.iclr_proprio_decoder = _action_head_constructor(
        ...
        self.iclr_visual_trace_decoder = _action_head_constructor(
            ...
            input_dim=self.latent_dim,
            hidden_features=self.decoder_hidden_features,
            output_dim=self.num_visual_trace_points * 2, # this is where we are predicting just a single step visual trace
            loss_fn=losses[self.sa_loss_fn](reduction="none"),
        )

        self.iclr_action_decoder = _action_head_constructor(
            head_type=self.decoder_pred_head,
            input_dim=self.latent_dim,
            hidden_features=self.decoder_hidden_features,
    ...
    def preprocessing(
        ...
        action : torch.Tensor,
        visual_trace : torch.Tensor,
    ) -> torch.Tensor:
        """
        This function preprocesses the observation, proprioception, and action data for the ICLR model.
        ...
        f_obs = self.iclr_vision_norm(f_obs)
        if self.camera_pos_emb: # False
            f_obs = f_obs + self.iclr_camera_pos_emb # add camera positional embeddings
        
        if self.separate_camera_adapter: # True
            if self.remove_proprio:
                f_obs = [self.iclr_attn_pooling[i].forward_visual(f_obs[:, :, i]) for i in range(self.num_cameras)]
            else:
                f_obs = [self.iclr_attn_pooling[i].combine_forward(f_obs[:, :, i], f_prop) for i in range(self.num_cameras)]    # this step combines the visual tokens and the proprio tokens
            f_s = torch.concat(f_obs, dim=-1)
            # pad at the last dimension
            if self.padding > 0: # padding is 0, so this is not executed
            ...
            # state extraction 
            if self.remove_proprio:
                f_s = self.iclr_attn_pooling.forward_visual(f_obs) # B, T, latent_len, self.latent_dim
            else:
                f_s = self.iclr_attn_pooling.combine_forward(f_obs, f_prop) # B, T, latent_len, self.latent_dim
        # f_s shape now is (batch size, sequence length, latent_len, 768), where latent_len is 1
        # action processing
        f_a = self.iclr_action_encoder(action) # B, T, self.latent_dim
        f_a = f_a[:, :, None, :] # B, T, 1, self.latent_dim

        # interweave state and action on the temporal dimension
        if self.modality_pos_emb:   # False
            f_s = f_s + self.iclr_pos_emb_state
            f_v = f_v + self.iclr_pos_emb_visual_trace
            f_a = f_a + self.iclr_pos_emb_act

        f_sva = torch.cat([f_s, f_v, f_a], dim=2) # B, T, latent_len+1+1, self.latent_dim
        f_sva = f_sva.view(f_sva.shape[0], -1, f_sva.shape[-1]) # B, T * (latent_len+1+1), self.latent_dim; T * (latent_len+1+1) is now 512 * (1 + 1 + 1)
        return f_sva

    def postprocessing(self, f_sa : torch.Tensor, B : int, T : int) -> tuple:   # never used
        """
        ...
        out_a = self.iclr_action_decoder(out_a).squeeze().view(B, T, self.action_dim * self.num_pred_steps).view(B, T, self.num_pred_steps, self.action_dim)
        
        # decoding to proprio
        if not self.pred_action_only:
            out_proprio = self.iclr_proprio_decoder(out_s).view(B, T - 1, self.num_pred_steps, self.proprio_dim)
            out_eos = out_a[:, :, :, -1]
            out_action = out_a[:, :, :, :-1]
        else:
            out_proprio = None
    ...
    def preprocessing_attention(self,   # never used, as forward_attention is not used
        observation : torch.Tensor, 
        proprio : torch.Tensor,
        ...
        f_obs = f_obs.view(f_obs.shape[0], f_obs.shape[1], -1, f_obs.shape[-1]) # B, T, N*K, self.vision_out_dim
        f_s = self.iclr_attn_pooling.forward_attention(f_obs, f_prop) # B, T, latent_len, self.latent_dim
        
        return f_s

    def forward_vision_attention(self, sequences : torch.Tensor) -> torch.Tensor:   # never used
    ...
    def forward(self, sequences : dict) -> torch.Tensor:
        ...
        if self.pred_action_only:   # True
            # remove eos 
            action = action[..., :self.action_dim]  # (seq_length, num_pred_steps, 10)
        
        # get dimensions 
        B, T = observation.shape[:2]    # (batch_size, seq_length)
        ...
        if self.random_mask_visual_trace:
            visual_trace_mask = 1. - prompt_mask
        ...
        # trying no_prompt_loss, original code is using mask = None
        if self.no_prompt_loss:
            B, seq_length = f_sva.shape[0], f_sva.shape[1]
            mask = torch.full((B, 1, seq_length, seq_length), float(1.0), device=f_sva.device)
            mask = torch.triu(mask, diagonal=1).type_as(f_sva)
            reshaped_prompt_mask = prompt_mask.repeat_interleave(3,dim=1)[:,None,None,:].repeat(1,1,seq_length,1)
            mask = mask * reshaped_prompt_mask
            mask = mask.masked_fill(mask == 1, float('-inf'))
        else:
            mask = None

        # mask = None   # original code is using mask = None

        # forward LM 
        out = self.llama(f_sva,mask) # B, T * (latent_len+1+1), self.latent_dim
        ...
        if not self.pred_action_only:    # self.pred_action_only is True, so we don't predict proprio
            # predict proprio as well
            proprio_target = proprio[:, 1:, :].reshape(B*(T-1), self.proprio_dim * self.num_pred_steps) 
            state_loss = self.iclr_proprio_decoder.loss(out_s, proprio_target)
        else:
            state_loss = torch.tensor(0.0, dtype=proprio.dtype,device=proprio.device)

        if self.scale_action is not None:   # self.scale_action is None
            action_target = scale_action(action, self.scale_action)
        
        visual_trace_target = visual_trace[:, :, 0, :].reshape(B*T, self.num_visual_trace_points * 2) 
        visual_trace_loss = self.iclr_visual_trace_decoder.loss(out_v, visual_trace_target)

        action_target = action.reshape(B*T, self.action_dim * self.num_pred_steps)
        action_loss = self.iclr_action_decoder.loss(out_a, action_target)
        ...
        if self.no_prompt_loss: # False
            ...
            visual_trace_loss = visual_trace_loss * prompt_mask.view(B*T,1)

        if self.step_weight > 1: # False
        ...
        if self.no_prompt_loss: # False
            ...
            visual_trace_loss = visual_trace_loss.mean()
            action_loss = action_loss.mean()

        # calculate total loss (weighted sum of action, proprio, and kl loss)
        loss = self.loss_w_action * action_loss + self.loss_w_proprio * state_loss + self.loss_w_visual_trace * visual_trace_loss + self.loss_w_kl * kl_loss
        loss = self.scale_loss * loss
        loss_dict = {
            "action_loss": action_loss,
            "state_loss": state_loss,
            "visual_trace_loss": visual_trace_loss,
            "kl_loss": kl_loss,
            "loss": loss, 
        }
    ...
    def get_new_parameters(self):
        for name, param in self.named_parameters():
            if "iclr" in name:  # new parameters have "iclr" in their names
    ...
    def forward_inference(self, sequences : dict, start_pos: int):
        ...
        action: B, T - 1 or T, action_dim (batch size, timesteps, action_dim) # we might not have action for the last state
        ...
        """
        observation = sequences['observation']  # in __call__, size is (1, 1, 2, 3, 224, 224) in the first loop, (1, 2, 2, 3, 224, 224) from the second loop
        proprio = sequences['proprio']
        visual_trace = sequences['visual_trace']
        action = sequences.get("action", None)  # in __call__, None in the first loop, (1, 1, 11) from the second loop
        # prompt_mask = sequences.get("prompt_mask", None)  # never used

        # Remove eos if predicting action only, in __call__, action is not None from the second loop
        if action is not None and self.pred_action_only:
            action = action[..., :self.action_dim]
        
        # get batch size and sequence length (of observation, proprio)
        B, T = observation.shape[:2]    # (1, 1) in the first loop because self.last_observation is None, (1, 2) from the second loop
        ...
        # If there is no action for the last state, we increase the action temporal length by 1
        if action_T == T - 1:   # this always happens in __call__
            if action is None: # if there is no action, then action is a tensor of zeros. This happens in __call__ of the first loop
                action = torch.zeros(B, 1, self.action_dim, dtype=proprio.dtype).to(device=proprio.device)
            else:    # if there is action, then increase the action temporal length by 1, by padding with zeros
                action = torch.concatenate([action, torch.zeros(B, 1, self.action_dim).to(device=action.device)], dim=1)
                # action shape is (1, 2, 10) in __call__ from the second loop

        # preprocessing observations, proprio and actions
        f_sva = self.preprocessing(observation, proprio, action, visual_trace) # B, T * (latent_len+1+1), self.latent_dim
        ...
        # forward LM, forward the f_sa preprocessed input to the llama model
        with torch.amp.autocast('cuda', dtype=torch.bfloat16):  # automatic mixed precision
            out = self.llama.forward_inference(f_sva, start_pos, mask) # B, T * (latent_len+1+1), self.latent_dim

        # parse the latent state of the model 
        out = out.view(B, T, -1, self.latent_dim) # (B, T, latent_len+1+1, self.latent_dim)

        if action_T == T - 1:   # if there is no action for the last state. This always happens in __call__
            # predicting the missing action
            ...
            else:   # currently we do this for action only
                # there's no eos prediction 
                out_eos = None
                out_action = out_a

            if self.scale_action is not None: # currently we do not do this 
            ...
            return out_action, out_eos, out_visual_trace
        ...
        #     out_proprio = self.iclr_proprio_decoder.pred(out_s).view(B, self.num_pred_steps, self.proprio_dim)
        #     return out_proprio.float()

    # Now, we are writing a separate function for the case of not using teacher forcing.
    @torch.inference_mode()
    def forward_inference_no_teacher_forcing(self, sequences : dict, start_pos: int, pred_visual_trace: bool = True):
        """
        observations: B, T, N, 3, H, W (batch size, timesteps, num_cameras, 3, height, width)
        proprio: B, T, proprio_dim (batch size, timesteps, proprio_dim)
        action: B, T - 1 or T, action_dim (batch size, timesteps, action_dim) # we might not have action for the last state
        ...
        observation = sequences['observation']  # in __call__, size is (1, 1, 2, 3, 224, 224) in the first loop, (1, 2, 2, 3, 224, 224) from the second loop
        proprio = sequences['proprio']
        visual_trace = sequences.get("visual_trace", None)
        action = sequences.get("action", None)  # in __call__, None in the first loop, (1, 1, 11) from the second loop
        # prompt_mask = sequences.get("prompt_mask", None)  # never used
        ...
        # padding actions so interweaving works 
        action_T = action.shape[1] if action is not None else 0
        visual_trace_T = visual_trace.shape[1] if visual_trace is not None else 0
        assert T == action_T + 1, f"Action temporal length differs by {T - action_T}"
        assert T == visual_trace_T + 1, f"Visual trace temporal length differs by {T - visual_trace_T}"

        if visual_trace is None:   # if there is no visual_trace, then visual_trace is a tensor of zeros. This happens in __call__ of the first loop
            visual_trace_temp = torch.zeros(B, 1, self.num_visual_trace_points * 2, dtype=proprio.dtype).to(device=proprio.device)
        else:   # if there is visual_trace, then increase the visual_trace temporal length by 1, by padding with zeros
            visual_trace_temp = torch.concatenate([visual_trace, torch.zeros(B, 1, self.num_visual_trace_points * 2).to(device=visual_trace.device)], dim=1)
            # visual_trace shape is (1, 2, 32) in __call__ from the second loop, after reshaping
        ...
        # preprocessing observations, proprio, visual_trace, and actions
        f_sva = self.preprocessing(observation, proprio, action, visual_trace_temp) # B, T * (latent_len+1+1), self.latent_dim
        mask = None
        ...
        # out_v_latent = out[:, -1, :1, :].view(B, self.latent_dim)
        # out_visual_trace = self.iclr_visual_trace_decoder.pred(out_v_latent)    # B, self.num_visual_trace_points * 2
        # out_visual_trace = out_visual_trace.float()
        if pred_visual_trace:
            out_v_latent = out[:, -1, :1, :].view(B, self.latent_dim)
            out_visual_trace = self.iclr_visual_trace_decoder.pred(out_v_latent)    # B, self.num_visual_trace_points * 2
            out_visual_trace = out_visual_trace.float()
            
            # =================================================================================================
            # compute the new visual trace to predict the action
            if visual_trace_temp.shape[1] == 1:
                visual_trace = out_visual_trace.unsqueeze(1)
            else:
                visual_trace = torch.concatenate([visual_trace, out_visual_trace.unsqueeze(1)], dim=1)

            # second turn of preprocessing observations, proprio, visual_trace, and actions
            f_sva = self.preprocessing(observation, proprio, action, visual_trace) # B, T * (latent_len+1+1), self.latent_dim
            mask = None
            ...
            out_visual_trace = torch.zeros(B, self.num_visual_trace_points * 2, dtype=out.dtype, device=out.device)

        # predicting the missing action
        out_a_latent = out[:, -1, 1:-1, :].view(B, (self.latent_len - 1 - 1) * self.latent_dim)
        out_a = self.iclr_action_decoder.pred(out_a_latent).view(B, self.num_pred_steps, self.action_dim)
    ...
    def reset(self, action_exec_horizon : int = 1) -> None:
        ...
        self.start_pos = 0
        self.last_visual_trace = None
        self.last_action = None
        self.last_observation = None    # set self.last_observation to None
        self.first_obs = True
    ...
    def prompt(self, prompt_sequences : dict) -> None:
        ...
        # if prompt_sequences["action"].shape == 4:   # False
        #     prompt_sequences["action"] = prompt_sequences["action"][:, :, 0]
        # if prompt_sequences["proprio"].shape == 4:   # False
        #     prompt_sequences["proprio"] = prompt_sequences["proprio"][:, :, 0]

        prompt_len = prompt_sequences["action"].shape[1] + prompt_sequences["observation"].shape[1] + prompt_sequences["visual_trace"].shape[1] # length of action can be one less than the length of observation
        ...
        #     )
        self.forward_inference(prompt_sequences, self.start_pos)    # in prompting, self.start_pos is now 0. And here we don't assign anything to the output
        self.start_pos += prompt_len    # update self.start_pos by the length of the prompt sequence
        self.first_obs = True   #  tracks whether the current observation is the first one after a prompt sequence has been processed
    ...
    def get_action_eval(self, observation, abs_gripper_control=False, use_temporal=True, binary_gripper=False, teacher_forcing=False):
        ...
        # observation is a dictionary of "observation", "proprio", "action",
        # including one side_image, one wrist_image, one proprio, and None or one action
        tmp = observation.copy()    # shallow 
        if teacher_forcing: # when teacher_forcing is True, gt_action is the action from the ground truth
            gt_action = observation["action"]   # can be None in the first loop
        ...
        # Following if-else overwrites the action in the observation dictionary, except the first loop, where observation["action"] is None
        if teacher_forcing:
            if observation["action"] is not None:   # if using teacher forcing, and action is not None (from the second loop)
                # This is essentially just observation["action"]
                observation["action"] = torch.cat([gt_action, torch.zeros((1, 1, 1), device=observation["action"].device)], dim=-1)   # add eos to gt_action, but will be ignored if prediction action only is True
        else:   # else then we use the last action to predict the current action
            observation["action"] = self.last_action

        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
            action_sequence, eos_sequence, visual_trace_sequence = self.forward_inference(observation, self.start_pos)
    ...
    # Now, we are writing a separate function for the case of not using teacher forcing.
    @torch.inference_mode()
    def get_action_eval_no_teacher_forcing(self, observation, abs_gripper_control=False, use_temporal=True, binary_gripper=False, pred_visual_trace=True):
        # if abs_gripper_control, then the current model prediction is used instead of the averaged gripper action
